...
Généré automatiquement par `scripts/generer_resultats.py` à partir des CSV produits par `scripts/fig1_sensibilites_pdi_autocall.py`, `fig2_autocall_vs_decrement.py` et `fig3_volatility_target.py` (seed globale unique, `src/marche.py::SEED_GLOBAL`). **Ne pas éditer à la main** : relancer `python scripts/run_all.py` pour tout régénérer si un paramètre ou une seed change.
